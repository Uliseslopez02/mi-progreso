-- Fase 2b (Agenda: conexión hábito↔agenda) — vínculo opcional a un hábito y modo de
-- completado en planner_items. Ver src/domain/types.ts (`PlannerItem.linkedHabitId`/
-- `habitCompletionMode`). Campos nullable, sin migración de dominio.

alter table public.planner_items
  add column linked_habit_id text,
  add column habit_completion_mode text;

-- Actualizar save_app_data para incluir linked_habit_id/habit_completion_mode de planner_items.
