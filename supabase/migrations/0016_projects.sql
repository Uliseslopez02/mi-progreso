-- Fase 5a (Proyectos + Kanban) — proyectos y sus tareas, sin fecha propia.
-- Ver src/domain/types.ts (`Project`, `ProjectTask`).
...
create policy "projects: owner rw" on public.projects
  for all to authenticated
  using (user_id = auth.uid()) with check (user_id = auth.uid());

-- Tarjeta de un tablero Kanban. FK compuesta a projects(user_id, id) para que
-- borrar un proyecto se lleve puestas sus tareas sin delete-diff manual aparte.
...
create index project_tasks_user_id_idx on public.project_tasks (user_id);
create index project_tasks_project_id_idx on public.project_tasks (user_id, project_id);

alter table public.project_tasks enable row level security;

create policy "project_tasks: owner rw" on public.project_tasks
  for all to authenticated
  using (user_id = auth.uid()) with check (user_id = auth.uid());

-- Actualizar save_app_data / clear_app_data para incluir projects/project_tasks.
...
begin
  ...
  delete from life_wheel_snapshots where user_id = uid;
