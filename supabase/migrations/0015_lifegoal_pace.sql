-- Fase 3a (Objetivos con ritmo propio) — tipo de progreso y ritmo en life_goals.
-- Ver src/domain/types.ts (`LifeGoal.kind`/`currentValue`/`targetValue`/`unit`/
-- `milestones`). Campos nullable/con default, sin migración de dominio.
...
-- Actualizar save_app_data para incluir los campos nuevos de life_goals.
