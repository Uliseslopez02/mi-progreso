-- Fase 7b (Rediseño Objetivos/Hábitos) — dificultad de un hábito (cosmética,
-- no afecta racha/consistencia/progreso). Ver src/domain/types.ts (`Goal.difficulty`).

alter table public.goals add column if not exists difficulty text;
...
begin
  ...
  insert into goals (id, user_id, category_id, name, weight, active, period, kind, target_value, unit, days_of_week, tracking_kind, frequency, difficulty, order_index, created_at)
  select
