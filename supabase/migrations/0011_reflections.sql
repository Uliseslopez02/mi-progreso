-- Fase 3 (Momento Mori) — birth_date/life_expectancy_years en user_settings +
-- tabla de reflexiones. Ver src/domain/types.ts (`Settings`, `Reflection`).

alter table public.user_settings
  add column birth_date date,
  add column life_expectancy_years int check (life_expectancy_years is null or life_expectancy_years between 1 and 130);
...
create policy "reflections: owner rw" on public.reflections
  for all to authenticated
  using (user_id = auth.uid()) with check (user_id = auth.uid());

-- Actualizar save_app_data / clear_app_data para incluir settings nuevos + reflections.
