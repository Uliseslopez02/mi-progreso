-- Fase 7a (Rediseño Objetivos/Hábitos) — eliminación completa de la Rueda de la vida.
-- Redefine save_app_data/clear_app_data sin life_wheel_snapshots y recién después
-- dropea la tabla (RLS/índice caen solos, sin necesidad de cascade).
--
-- Importante: save_app_data se redefinió por última vez en 0019_nav_order.sql,
-- pero clear_app_data por última vez en 0017_notes.sql — no son la misma base,
-- así que cada una parte de su propia versión más reciente. De paso se corrige un
-- gap preexistente (no de esta fase): clear_app_data nunca reseteaba nav_order,
-- porque esa columna no existía todavía cuando 0017 la escribió por última vez.
...
begin
  ...
  insert into user_settings (user_id, app_name, streak_threshold, allow_editing_past_days, birth_date, life_expectancy_years, nav_order, updated_at)
  values (
    uid,
    ...
    array(select jsonb_array_elements_text(coalesce(payload->'settings'->'navOrder', '[]'::jsonb))),
    now()
  )
  ...
  insert into categories (id, user_id, name, order_index, color)
  select c->>'id', uid, c->>'name', coalesce((c->>'order')::int, 0), c->>'color'
...
-- Recién ahora, con ninguna función referenciando la tabla, se puede dropear.
drop table public.life_wheel_snapshots;
