-- Fase 3 (Rueda de la vida) — snapshots 1-10 por categoría en una fecha.
-- Ver src/domain/types.ts (`LifeWheelSnapshot`, `LifeWheelAreaScore`).

create table public.life_wheel_snapshots (
  id          text not null,
...
create index life_wheel_snapshots_user_id_idx on public.life_wheel_snapshots (user_id);

alter table public.life_wheel_snapshots enable row level security;

create policy "life_wheel_snapshots: owner rw" on public.life_wheel_snapshots
  for all to authenticated
  using (user_id = auth.uid()) with check (user_id = auth.uid());

-- Actualizar save_app_data / clear_app_data para incluir life_wheel_snapshots.
