-- 0027_fix_save_app_data_life_wheel.sql
-- Bug: save_app_data (redefinida en 0024_plan_limits.sql, copiada de 0017_notes.sql)
-- todavía tenía el insert/delete de `life_wheel_snapshots`, tabla eliminada por
-- 0020_drop_life_wheel.sql. Cualquier guardado (save_app_data) fallaba con
-- `relation "life_wheel_snapshots" does not exist` (42P01), que PostgREST
-- traduce a 404 — rompía el guardado para TODAS las cuentas desde que se
-- aplicó 0024 (2026-09-15). clear_app_data ya estaba corregida (0025_notifications.sql).
--
-- Esta es la misma save_app_data de 0024_plan_limits.sql, sin los dos bloques
-- de life_wheel_snapshots (insert...on conflict... y delete...where...).
--
-- Ya aplicada manualmente en Supabase producción (2026-09-17) por ser un bug
-- activo que rompía el guardado de datos de todos los usuarios; verificado con
-- un save_app_data real (204) desde una cuenta de prueba. Esta migración deja
-- el cambio registrado en el repo.
...
--     (select pg_get_functiondef(p.oid) from pg_proc p where p.proname = 'save_app_data'),
--     chr(10)
--   )) as line where line ilike '%life_wheel%';
--   -- debe dar 0
